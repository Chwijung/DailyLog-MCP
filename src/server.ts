  ...
  server.registerTool(
    "submit_daily_log",
    {
      ...
      inputSchema: {
        title: z
          .string()
          .describe("작업 요약 제목 (최대 40자). 성과가 드러나게. 예: 팀원 로그 공유로 협업 가시성 개선"),
        feature_name: z
          .string()
          .describe("작업한 기능명 (최대 100자). 예: 데일리 로그 팀 단위 조회(읽기 전용)"),
        problems: z
          .string()
          .describe(
        ...
        content: z
          ...
          .describe(
            "상세 내용 (선택, 최대 6000자, 마크다운 가능). " +
              ...
              "그래서 content에는 '문제/해결/결과' 같은 헤더를 다시 달지 말고, 한 편의 흐르는 글로 쓰세요. " +
              "첫 문단(3~5줄)에 핵심을 front-load(포트폴리오 생성기는 plain text로 바꿔 앞 600자만 사용, 무거운 HTML 서식은 안 보임). " +
